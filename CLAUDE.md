...
**Fonte de dados**: `src/data/anunciantes.js`, exportando um array `anunciantes`. Cada item (ver comentário completo no topo do arquivo, que é a referência definitiva — o resumo abaixo pode ficar desatualizado se o arquivo mudar):

```js
{
    id,        // string, identificador único
    nome,      // string. Tipo 'card': sempre exibido. Tipo 'imagem': sobreposto na imagem (com degradê escuro por trás) só se `nome`, `descricao` ou algum botão (whatsapp/site) estiver preenchido.
    descricao, // string. Mesma regra de exibição do `nome`, nos dois tipos.
    cor,       // string hex, cor de marca do anunciante (ex: '#0F6E56') — usada no bloco superior (fallback sólido sem `imagem`, e como degradê nas bordas quando há `imagem`) e nos botões de CTA. Vale para tipo === 'card' e para tipo === 'imagem'.
    imagem,    // caminho dentro de public/anunciantes/, nunca URL externa. Opcional em tipo === 'card' (sem ela, usa `cor` sólida, sem ícone); obrigatória em tipo === 'imagem' (cobre o bloco inteiro).
    categoria, // 'empresa' | 'produto' | 'servico', ou '' quando não categorizado — só organizacional, não afeta renderização
    destaque,  // string, ou '' quando não aplicável; só lido no tipo 'card' — sem efeito em tipo 'imagem', usar ''
    tipo,      // 'card' ou 'imagem' (default 'card')
    posicaoX,  // número, 0 quando no padrão; desloca o enquadramento horizontal de `imagem`. 0 = centralizado, negativo = mostra mais a ESQUERDA, positivo = mais a DIREITA. Convertido para background-position via `50 + posicaoX` (limitado a 0-100).
    posicaoY,  // número, 0 quando no padrão; mesma lógica de `posicaoX`, no eixo vertical. 0 = centralizado, negativo = mostra mais a parte de CIMA, positivo = mais a parte de BAIXO. `50 + posicaoY` (0-100). Também é o ponto de origem do `zoom` abaixo (junto com `posicaoX`).
    zoom,      // número >= 0, 0 quando sem zoom extra (só o cover mínimo). Não aceita negativo (ficaria abaixo do cover mínimo, o que deixaria espaço em branco — matematicamente inevitável, por isso não é uma opção). Escala aplicada = 1 + (zoom / 100), ampliando em torno de `posicaoX`/`posicaoY`.
    whatsapp, site, ativo
}
```

**Regra de chaves padronizadas**: todo registro deve declarar todas as chaves acima, na mesma ordem, usando valor neutro quando não aplicável ao tipo ou quando no padrão (`''` para `categoria`/`destaque`, `0` para `posicaoX`/`posicaoY`/`zoom`) — nunca omitir uma chave que exista em outro registro. Vale tanto para `anunciantes.js` quanto para `anunciantes.exemplo.js`; os 6 registros atuais de `anunciantes.exemplo.js` seguem essa regra.

O campo `icone` **não existe** nesse array — é usado só internamente por `AdBanner.vue` para o slide fixo de "vaga disponível" (hardcoded no componente, não lido de nenhum registro real). Os campos `foto`/`imagemUrl` (um por tipo) **foram unificados** em `imagem` (usado pelos dois tipos, mesmo significado); `titulo`/`subtitulo` (exclusivos do tipo `imagem`) **foram unificados** com `nome`/`descricao` (que já existiam para o tipo `card`) — hoje há um único par de campos para os dois tipos, só o comportamento de exibição continua diferente por tipo. Hoje o array está **vazio** — nenhum anunciante real cadastrado ainda.

**Imagens de anunciantes**: `imagem` deve apontar para caminhos dentro de `public/anunciantes/` (ex: `/anunciantes/ocelos-pesca.jpg`), **nunca URLs externas**. A pasta `public/anunciantes/` existe (com um `.gitkeep` para ficar versionada mesmo vazia) e não tem subpastas por cliente — os arquivos são nomeados pelo `id` do anunciante. Se o mesmo anunciante tiver imagem de card **e** banner (`tipo: 'imagem'`), diferenciar o nome (ex: `ocelos-pesca.jpg` para a foto do card, `ocelos-pesca-banner.jpg` para o banner).

Existe também `src/data/anunciantes.exemplo.js`, exportando `anunciantesExemplo` — registros ilustrativos usados só para testar visualmente o componente, com cores bem distintas entre si e fotos reais em `public/anunciantes/` (não SVGs embutidos como em versões anteriores), cada uma com `posicaoX`/`posicaoY`/`zoom` ajustados observando o conteúdo real da foto. O campo `whatsapp` de exemplo usa um número claramente fictício (DDD `00`, que não existe no Brasil) — nunca um número real e discável — só para demonstrar o botão condicional; registros que não precisam demonstrar esse botão deixam `whatsapp` vazio. **Não é importado em nenhum lugar do código de produção** (fora do modo demo) — é usado pela rota `/publicidade` (ver abaixo) e, para testes manuais adicionais, pode-se trocar temporariamente o import de `anunciantes` por `anunciantesExemplo` em `AdBanner.vue`. Esse arquivo é usado ativamente para experimentação (registros e fotos são adicionados/removidos com frequência durante o desenvolvimento do visual) — não assumir que a lista de ids/arquivos aqui descrita é exaustiva ou definitiva; conferir o arquivo real antes de mexer.

**Regras de exibição** (`AdBanner.vue` filtra apenas os itens com `ativo: true`):

- **0 anunciantes ativos** (caso atual, array vazio): mostra um único slide fixo de "vaga disponível" — nome "Sua empresa aqui", descrição "Anuncie seu produto ou serviço para quem usa esta ferramenta", sem destaque, cor padrão verde (`#198754`) com o ícone `bi-megaphone` (esse é o único slide `card` que mostra ícone — é o próprio anúncio da ferramenta, sem cliente por trás), e um único botão "Quero anunciar" apontando para o WhatsApp de `config.js` (`whatsappLink`) — nunca para uma empresa inventada.
...
- **`tipo: 'card'`** (default): bloco superior de ~120px. Se `imagem` existir: cobre o bloco inteiro (`background-size: cover`) e a **própria foto** desaparece gradualmente nas bordas esquerda/direita — feito com `mask-image`/`-webkit-mask-image: linear-gradient(to right, transparent 0%, black 25%, black 75%, transparent 100%)` aplicado no elemento `.slide-photo-img` (preto = foto totalmente visível, transparente = foto totalmente invisível ali). Nesse caso `.slide-photo` **não recebe nenhuma cor de fundo** (nem `slide.cor` nem `COR_PADRAO`), então a máscara revela o branco do próprio `.card-body` por trás, e não uma cor sólida. Sem ícone. Se não houver `imagem`: `.slide-photo` recebe `background-color: slide.cor || COR_PADRAO` (cor sólida), sem ícone (exceto no slide de "vaga disponível", que sempre mostra `bi-megaphone`). Rótulo "Publicidade" pequeno no canto superior esquerdo. Abaixo do bloco: nome, descrição (2 linhas), linha de destaque, e **botões de CTA condicionais** — um botão WhatsApp (se `whatsapp` preenchido) e/ou um botão "Visitar site" (se `site` preenchido), lado a lado, largura proporcional ao texto (sem `w-100`); ambos usam a cor do anunciante como fundo, com cor do texto escolhida automaticamente para contraste (preto ou branco, conforme a luminância da cor — função `corTextoContraste`).
- **`tipo: 'imagem'`**: imagem de fundo (`imagem`) cobrindo o bloco inteiro (`background-size: cover`). Se `nome`, `descricao`, `whatsapp` ou `site` estiverem preenchidos: sobrepõe um degradê escuro sutil na parte inferior (de baixo para cima, `rgba(0,0,0,.65)` a transparente) para legibilidade, com nome (maior, negrito), descrição (menor) e os mesmos botões condicionais WhatsApp/Site do formato `card` (usando `cor` do anunciante), todos ancorados na parte inferior da imagem. Se nenhum desses 4 campos estiver preenchido: comportamento simples — a imagem inteira é um único link clicável (`site`, ou WhatsApp via `wa.me` como fallback), sem nenhum texto/cor/botão sobreposto. Não tem o rótulo "Publicidade".

**Enquadramento por imagem (`posicaoX`/`posicaoY`/`zoom`)**: a base do preenchimento é sempre `background-size: cover` (nunca sobra espaço em branco, independente do zoom). Por cima disso, o elemento que carrega a imagem (uma camada própria — `.slide-photo-img` no `card`, `.slide-image-img` no `imagem` — separada da camada de degradê/texto/botões sobrepostos, para eles não serem afetados pelo zoom) recebe `background-position: <X>% <Y>%` e `transform: scale(1 + zoom/100)` com `transform-origin: <X>% <Y>%`, onde `X = 50 + posicaoX` e `Y = 50 + posicaoY` (cada um limitado a 0-100) — ou seja, o zoom amplia em torno do mesmo ponto escolhido em `posicaoX`/`posicaoY`, não do centro fixo. Como o container (`.slide-photo`/`.slide-image-bg`) já tem `overflow: hidden`, o excesso da imagem ampliada é cortado automaticamente. Escala centrada em zero: `posicaoX`/`posicaoY` ausentes viram `0` (centralizado); negativo desloca para cima/esquerda, positivo para baixo/direita. `zoom` ausente ou negativo vira `0` (sem zoom extra — não é permitido reduzir abaixo do cover mínimo). Funções `clampEixo`/`clampZoom`/`estiloEnquadramento` em `AdBanner.vue`.

**abrir_email removido**: o botão de E-mail do estado "vaga disponível" foi removido numa revisão anterior (agora "Quero anunciar" único), e com ele o evento `abrir_email` (seção 9) deixou de ter qualquer disparo no código — só `abrir_whatsapp` continua ativo, disparado pelo botão do estado "vaga disponível" (cliques em anunciantes reais não emitem esse evento).

**Altura estável entre slides**: todo slide (incluindo o de "vaga disponível" e o `tipo: 'imagem'`, com ou sem texto/botões sobrepostos) fica dentro do mesmo wrapper `.slide-body`, com **altura fixa em CSS (`height: 260px`)** — o mesmo valor para os dois formatos, então trocar de slide no rodízio não muda a altura do bloco. Dentro do formato `card`, a linha de destaque **sempre existe no DOM**, mesmo vazia; quando não há destaque, fica com `visibility: hidden` (nunca removida via `v-if`), e a descrição é limitada a 2 linhas via `-webkit-line-clamp` com altura fixa (`height: 2.8em`). Nada disso usa JavaScript para medir ou calcular alturas — é só CSS. O valor `260px` foi medido empiricamente e não deve ser alterado sem novo teste visual.

**Card**: usa as mesmas classes dos componentes irmãos (`card border-1 mb-3` + `card-body`, aqui com `p-0` para o carrossel ocupar a largura/altura inteira até a borda), sem largura máxima customizada — igual a `TextInput`/`AvailableNumbers`/etc. `.card-body` tem `overflow: hidden` e `border-radius: inherit` (herda o raio de `.card`) — sem isso, fotos e imagens de fundo (que têm cantos quadrados) vazavam por trás dos cantos arredondados do card, deixando ver o fundo branco atrás deles.

**Histórico de bug — espaço branco residual na base do slide**: mesmo depois da correção acima, sobrava uma faixa branca abaixo da imagem em alguns anunciantes. A causa era `.card-body` ainda ter a classe `pb-3` (padding-bottom) sobrando de uma versão anterior do card — isso adicionava ~1rem de espaço em branco abaixo do carrossel inteiro (fora da área de 260px do `.slide-body`, mas dentro do card, então visível como uma faixa branca no rodapé). Corrigido removendo `pb-3` (`.card-body` ficou só `p-0 text-center`). Toda a cadeia de altura entre o container fixo de 260px (`.slide-body`) e o elemento de imagem (`.slide-photo-img`/`.slide-image-img`) não tem nenhum padding/margin — cada camada intermediária (`.slide-image-bg`, `.slide-photo`) usa `height: 100%` ou altura fixa igual, sem folga. Se uma faixa branca voltar a aparecer, o primeiro lugar a checar é padding/margin acumulado nesse caminho, não o `background-size: cover` (que já garante preenchimento total independente da proporção da imagem).

**Histórico — degradê de cor sobreposta trocado por máscara de opacidade**: a versão anterior do fade nas bordas do slide `card` usava um elemento `.slide-photo-gradient` por cima da foto, com `background: linear-gradient(to right, <cor> 0%, transparent 25%, transparent 75%, <cor> 100%)` — uma camada de cor sólida com opacidade decrescente, que podia se misturar visualmente com a foto por baixo. Trocado por uma máscara de opacidade aplicada na própria foto (`.slide-photo-img`, não mais um elemento extra): `mask-image`/`-webkit-mask-image: linear-gradient(to right, transparent 0%, black 25%, black 75%, transparent 100%)` (preto = foto visível, transparente = foto invisível ali), fazendo a foto em si desaparecer nas bordas, sem nenhuma camada de cor sobreposta. Só se aplica ao tipo `card` (`.slide-photo-img`) — o tipo `imagem` (`.slide-image-img`) não tem esse fade nas bordas; ele usa um degradê escuro (`.slide-image-overlay`) só na base, para legibilidade do texto sobreposto, o que é um mecanismo diferente e não foi alterado.

**Correção — cor sólida atrás da foto anulava a transparência da máscara**: na primeira versão da máscara acima, `.slide-photo` sempre recebia `background-color: slide.cor || COR_PADRAO`, independente de haver `imagem` ou não. Como a máscara só afeta a opacidade da própria foto (`.slide-photo-img`), essa cor sólida ficava visível por trás nas bordas em vez de revelar o fundo branco do card — ou seja, a foto não desaparecia de verdade, só trocava de cor. Corrigido: `.slide-photo` só recebe `background-color` quando **não** há `slide.imagem` (caso do anunciante sem foto, que precisa de algum fundo visível com o ícone/cor de marca); quando há `imagem`, `.slide-photo` fica sem nenhuma cor de fundo, e a máscara revela o branco do `.card-body` por trás. O uso de `slide.cor` em `ctaStyle` (botões de CTA) não foi alterado — só o fundo do bloco de foto.

**Navegação via Bootstrap Carousel**: a marcação usa `.carousel`/`.carousel-inner`/`.carousel-item`/`.carousel-indicators`/`.carousel-control-prev`/`.carousel-control-next` padrão do Bootstrap 5, com `id="ad-carousel"`.

- **Setas**: sem override de altura — usam o comportamento padrão do Bootstrap (centralizadas verticalmente em toda a altura do card, 260px). O ícone (`.carousel-control-prev-icon`/`-next-icon`) ganhou um fundo circular escuro semi-transparente (`rgba(0,0,0,.35)`, `border-radius: 50%`) para ficar visível mesmo sobre fotos claras.
- **Indicadores**: restilizados como pontinhos brancos pequenos dentro de uma "pílula" escura semi-transparente (mesmo espírito visual das setas), **sempre sobrepostos e sempre na base da área visual** (`position: absolute`, `z-index: 5`, nunca no meio da altura). Como o Bootstrap só tem um único elemento `.carousel-indicators` compartilhado por todos os slides (não um por item), a posição depende do **tipo do slide ativo no momento**: por padrão (`bottom: 150px`, medido a partir da base do `.slide-body` de 260px) fica colado na base do bloco de foto de 120px dos slides `card`; quando o slide ativo é `tipo: 'imagem'`, uma classe `.tipo-imagem-ativo` no `#ad-carousel` troca para `bottom: 10px` — colado na base da imagem inteira, acima da faixa de nome/descrição/botões (que ganhou `padding-bottom: 34px` para não ficar embaixo dos indicadores). Essa classe é atualizada reativamente (`tipoAtivo`, um `ref`) ouvindo o evento nativo `slide.bs.carousel` do Bootstrap (`e.to` dá o índice do próximo slide) — não há outra forma de saber, via CSS puro, qual é o tipo do slide atualmente ativo num carrossel com indicadores compartilhados.
- **Arrastar com mouse**: o Carousel do Bootstrap só arrasta por toque real (`touch`). Foi adicionado suporte a arrastar com o botão do mouse via Pointer Events no elemento `#ad-carousel` (`pointerdown`/`pointermove`/`pointerup`/`pointercancel`), ignorando `pointerType === 'touch'` (não duplica o gesto nativo do Bootstrap). Durante o `pointermove` (arrasto em andamento), aplica `transform: translateX()` diretamente (via DOM, não reatividade do Vue — é um gesto de alta frequência) no `.slide-body` do slide ativo, proporcional ao deslocamento horizontal, com resistência leve acima de 80px (`ARRASTO_MAX`) pra não deslizar indefinidamente. Ao soltar (`pointerup`): se o deslocamento passou de 50px (`LIMIAR_ARRASTO`), chama `carouselInstance.next()`/`.prev()` e zera o transform manual sem transição (a troca de slide do Bootstrap assume a transição visual); se não passou, anima o transform de volta a `0` com `transition: transform 0.25s ease`. **Importante**: o handler ignora `pointerdown` que comece dentro de um `<a>`/`<button>` (`e.target.closest('a, button')`) — sem isso, `setPointerCapture` no `pointerdown` capturava o ponteiro para o carrossel e **quebrava o clique nativo nas setas/indicadores/botões de CTA** (bug real encontrado e corrigido durante esta revisão).

A instância JS (`new Carousel(el, { interval: 7000, ride: 'carousel', pause: 'hover', touch: true, wrap: true })`) é criada/destruída via um `watch(() => props.visible, ...)` em vez de `onMounted`/`onUnmounted` simples — necessário porque o `v-if="visible"` fica **dentro** do próprio template do `AdBanner` (mesmo padrão de `AvailableNumbers`/`UnpaidNumbers`/`DrawHistory`, seção 24), então o componente Vue monta uma vez só (na carga da página) e o elemento do carrossel só existe no DOM quando `visible` fica `true` depois. Usar `onMounted` puro tentaria instanciar o Carousel antes de o elemento existir (outro bug real encontrado e corrigido numa revisão anterior: a rotação automática simplesmente não iniciava). Isso também corrige a documentação: o embaralhamento da lista de anunciantes (Fisher-Yates) acontece no `setup()` do componente — ou seja, **uma vez por carregamento de página**, não a cada vez que `visible` fica `true` (a instância do componente não é recriada quando `numerosDisponiveis` some e volta).
...
- `AdBanner.vue` usa `anunciantesExemplo` (`src/data/anunciantes.exemplo.js`) no lugar de `anunciantes` — os anúncios ilustrativos, não os reais.
