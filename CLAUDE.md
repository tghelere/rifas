...
{
    id, nome, descricao,
    icone,     // classe do Bootstrap Icons (ex: 'bi-shop'). Usada SÓ no slide especial de "vaga disponível" — anunciantes reais sem foto mostram apenas a cor sólida, sem ícone.
    cor,       // string hex, cor de marca do anunciante (ex: '#0F6E56') — usada no bloco superior (fallback sem foto, e como degradê nas bordas quando há foto) e nos botões de CTA. Vale para tipo === 'card' e para tipo === 'imagem' (quando há botões/título/subtítulo sobrepostos).
    foto,      // opcional; caminho de imagem de fundo do bloco superior (cobre o bloco inteiro, com degradê da `cor` nas bordas esquerda/direita). Se ausente, usa `cor` sólida, sem ícone. Só usada quando tipo === 'card'.
    ...
    titulo,    // opcional; só usado quando tipo === 'imagem' — sobreposto na imagem, com degradê escuro por trás
    subtitulo, // opcional; só usado quando tipo === 'imagem' — sobreposto abaixo do título
    posicaoY,  // opcional (0-100, default 50); vale para `foto` OU `imagemUrl`. 0 = topo da imagem, 50 = centralizado, 100 = base. Também é o ponto de origem do `zoom` abaixo.
    zoom,      // opcional (número, default 100, mínimo 100); vale para `foto` OU `imagemUrl`. 100 = sem zoom extra; 130 = amplia 30% além do cover mínimo, em torno de `posicaoY`.
    whatsapp, site, ativo
}
```
...
Existe também `src/data/anunciantes.exemplo.js`, exportando `anunciantesExemplo` — registros ilustrativos usados só para testar visualmente o componente, com cores bem distintas entre si (`#0F6E56` verde-petróleo, `#0D6EFD` azul, `#C1440E` terracota, `#0E7C7B` verde-azulado no tipo `imagem`) e fotos reais em `public/anunciantes/` (não SVGs embutidos como em versões anteriores), cada uma com `posicaoY`/`zoom` ajustados observando o conteúdo real da foto. **Não é importado em nenhum lugar do código de produção** (fora do modo demo) — é usado pela rota `/publicidade` (ver abaixo) e, para testes manuais adicionais, pode-se trocar temporariamente o import de `anunciantes` por `anunciantesExemplo` em `AdBanner.vue`. Esse arquivo é usado ativamente para experimentação (registros e fotos são adicionados/removidos com frequência durante o desenvolvimento do visual) — não assumir que a lista de ids/arquivos aqui descrita é exaustiva ou definitiva; conferir o arquivo real antes de mexer.
...
**Card**: usa as mesmas classes dos componentes irmãos (`card border-1 mb-3` + `card-body`, aqui com `p-0 pb-3` para o carrossel ocupar a largura inteira até a borda), sem largura máxima customizada — igual a `TextInput`/`AvailableNumbers`/etc. `.card-body` tem `overflow: hidden` e `border-radius: inherit` (herda o raio de `.card`) — sem isso, fotos e imagens de fundo (que têm cantos quadrados) vazavam por trás dos cantos arredondados do card, deixando ver o fundo branco atrás deles (mesma causa do "espaço branco" percebido no rodapé do slide `tipo: 'imagem'`: não era falta de `cover`, era o canto quadrado da imagem aparecendo por trás do canto arredondado do card).
