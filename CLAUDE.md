...
Existe também `src/data/anunciantes.exemplo.js`, exportando `anunciantesExemplo` — 4 registros ilustrativos usados só para testar visualmente o componente: 3 do tipo `card` com cores bem distintas entre si (`#0F6E56` verde-petróleo, `#0D6EFD` azul, `#C1440E` terracota) e `foto` preenchida, e 1 do tipo `imagem` (`#0E7C7B`, com `titulo`/`subtitulo`/botões sobrepostos). Usa fotos reais em `public/anunciantes/exemplo-1.jpg` a `exemplo-4.jpg` (não SVGs embutidos como em versões anteriores). **Não é importado em nenhum lugar do código de produção** (fora do modo demo) — é usado pela rota `/publicidade` (ver abaixo) e, para testes manuais adicionais, pode-se trocar temporariamente o import de `anunciantes` por `anunciantesExemplo` em `AdBanner.vue`.
