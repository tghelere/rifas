...
```js
{
    ...
    destaque,  // opcional (string); só lido no tipo 'card' — não tem efeito em tipo 'imagem', não incluir nesse caso
    tipo,      // 'card' ou 'imagem' (default 'card')
    posicaoX,  // opcional (número, default 0); desloca o enquadramento horizontal de `imagem`. 0 = centralizado, negativo = mostra mais a ESQUERDA, positivo = mais a DIREITA. Convertido para background-position via `50 + posicaoX` (limitado a 0-100).
    posicaoY,  // opcional (número, default 0); mesma lógica de `posicaoX`, no eixo vertical. 0 = centralizado, negativo = mostra mais a parte de CIMA, positivo = mais a parte de BAIXO. `50 + posicaoY` (0-100). Também é o ponto de origem do `zoom` abaixo (junto com `posicaoX`).
    zoom,      // opcional (número >= 0, default 0); 0 = sem zoom extra (só o cover mínimo). Não aceita negativo (ficaria abaixo do cover mínimo, o que deixaria espaço em branco — matematicamente inevitável, por isso não é uma opção). Escala aplicada = 1 + (zoom / 100), ampliando em torno de `posicaoX`/`posicaoY`.
    whatsapp, site, ativo
}
```
