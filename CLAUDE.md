...
- **Indicadores**: restilizados como pontinhos brancos pequenos dentro de uma "pílula" escura semi-transparente (mesmo espírito visual das setas), **sobrepostos** sobre a área visual do slide (`position: absolute`, não mais `static`/abaixo em espaço branco) — fixados a `92px` do topo (perto da base do bloco de foto de 120px nos slides `card`). Como o Bootstrap só tem um único elemento `.carousel-indicators` compartilhado por todos os slides (não um por item), essa posição é um valor fixo pensado para funcionar nos dois formatos: fica logo abaixo do bloco de foto no formato `card`, e no formato `imagem` fica sobre a imagem, acima da faixa de texto/botões (que fica ancorada embaixo) — não é, tecnicamente, "colado na base" da imagem inteira nesse segundo caso, mas cumpre o requisito de nunca sobrepor o texto/botões e nunca cair em espaço branco.
- **Arrastar com mouse**: o Carousel do Bootstrap só arrasta por toque real (`touch`). Foi adicionado suporte a arrastar com o botão do mouse via Pointer Events (`pointerdown`/`pointerup` no elemento `#ad-carousel`), ignorando `pointerType === 'touch'` (não duplica o gesto nativo do Bootstrap) e comparando o deslocamento horizontal contra um limiar de 50px para decidir entre `carouselInstance.next()`/`.prev()`. **Importante**: o handler ignora `pointerdown` que comece dentro de um `<a>`/`<button>` (`e.target.closest('a, button')`) — sem isso, `setPointerCapture` no `pointerdown` capturava o ponteiro para o carrossel e **quebrava o clique nativo nas setas/indicadores/botões de CTA** (bug real encontrado e corrigido durante esta revisão).
